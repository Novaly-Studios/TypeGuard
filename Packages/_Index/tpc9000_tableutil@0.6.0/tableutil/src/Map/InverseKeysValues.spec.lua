return function()
    local InverseKeysValues = require(script.Parent.InverseKeysValues)

    describe("Map/InverseKeysValues", function()
        it("should return a blank table for no data", function()
            expect(next(InverseKeysValues({}))).never.to.be.ok()
        end)

        it("should return a single flipped key-value pair for a single item table", function()
            local Result = InverseKeysValues({X = "Y"})
        ...
        it("should return a flipped key-value pair for each item in the table", function()
            local Result = InverseKeysValues({A = "P", B = "Q", C = "R"})
