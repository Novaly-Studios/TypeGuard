...
return function()
    ...
    describe("Init", function()
        ...
        it("should serialize and deserialize a variety of numbers", function()
            ...
            expect(Base:Deserialize(Base:Serialize(-math.huge))).to.equal(-math.huge)
            expect(Base:Deserialize(Base:Serialize(0/0))).to.equal(0/0)
        end)
    end)
    ...
    describe("Integer", function()
        ...
        it("should serialize and deserialize various integers", function()
            ...
            expect(UInt32:Deserialize(Int32:Serialize(2^32-1))).to.equal(2^32-1)
        end)
    end)
    ...
    describe("Decimal", function()
        ...
        it("should serialize and deserialize various decimals", function()
            ...
            expect(Decimal:Deserialize(Decimal:Serialize(100.123))).to.equal(100.123)
            expect(Decimal:Deserialize(Decimal:Serialize((2^50)+0.123))).to.equal((2^50)+0.123)
        end)
    end)
    ...
    describe("IsNaN", function()
        ...
        it("should serialize and deserialize", function()
            ...
            expect(NaN:Deserialize(NaN:Serialize(0/0))).to.equal(0/0)
        end)
    end)
    ...
    describe("Dynamic", function()
        it("should serialize and deserialize", function()
            local DynamicInt = Base:Dynamic()
            local Sample1 = DynamicInt:Serialize(2^32-1)
            expect(buffer.len(Sample1)).to.equal(5)
            local Sample2 = DynamicInt:Serialize(-2^32-1)
            expect(buffer.len(Sample2)).to.equal(5)
            local Sample3 = DynamicInt:Serialize(2^24-1)
            expect(buffer.len(Sample3)).to.equal(4)
            local Sample4 = DynamicInt:Serialize(-2^24-1)
            expect(buffer.len(Sample4)).to.equal(4)
            local Sample5 = DynamicInt:Serialize(2^16-1)
            expect(buffer.len(Sample5)).to.equal(3)
            local Sample6 = DynamicInt:Serialize(-2^16-1)
            expect(buffer.len(Sample6)).to.equal(3)
            local Sample7 = DynamicInt:Serialize(2^8-1)
            expect(buffer.len(Sample7)).to.equal(2)
            local Sample8 = DynamicInt:Serialize(-2^8-1)
